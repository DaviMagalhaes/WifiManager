...
/** Wifi config page handler with response in JSON by Davi Magalhães */
void WiFiManager::handleWifiJson() {

  String page = FPSTR(HTTP_JSON_INIT);
  ...
  if (n == 0) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (_minimumQuality == -1 || _minimumQuality < quality) {
        String item = FPSTR(HTTP_JSON_ITEM);
        String rssiQ;
        rssiQ += quality;
        item.replace("{v}", WiFi.SSID(indices[i]));
        item.replace("{r}", rssiQ);
  ...
  page += FPSTR(HTTP_JSON_END);

  server->send(200, "application/json", page);

  DEBUG_WM(F("Sent config page / List of APs in JSON"));
}
